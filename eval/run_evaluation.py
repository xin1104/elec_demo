...
def main():
    ...
    if choice == "1":
        ...
        questions = generate_questions(api_config)
        if not questions:
            print("问题生成失败，完整评估流程已停止。")
            return

        print("\n2. 获取模型回答")
        from get_model_answers import main as get_answers_main

        if get_answers_main() is False:
            print("模型回答生成失败，完整评估流程已停止。")
            return

        print("\n3. 评估模型回答")
        from evaluate_answers import auto_evaluate

        if auto_evaluate(api_config) is False:
            print("自动评估失败，完整评估流程已停止。")
    elif choice == "0":
        print("退出程序")
        sys.exit()
    else:
